...
def add_expense(user_id):
    ...
    local_id = 1 if last_local_id is None else last_local_id + 1
...
def delete_expense(user_id):
    ...
    cursor.execute(
        "DELETE FROM expenses WHERE user_id=%s AND local_id=%s",
        (user_id, local_id)
    )
    conn.commit()
...
""")
        ...
        if choice == "1":
            query = "SELECT local_id, amount, category, description, date FROM expenses WHERE user_id=%s ORDER BY amount DESC"
        elif choice == "2":
            query = "SELECT local_id, amount, category, description, date FROM expenses WHERE user_id=%s ORDER BY amount ASC"
        elif choice == "3":
            query = "SELECT local_id, amount, category, description, date FROM expenses WHERE user_id=%s ORDER BY date DESC"
        elif choice == "4":
            query = "SELECT local_id, amount, category, description, date FROM expenses WHERE user_id=%s ORDER BY date ASC"
        elif choice == "5":
...
4. Main Menu
""")
                choice_view = input("Enter choice: ")
